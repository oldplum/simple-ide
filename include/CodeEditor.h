...
protected:
    void resizeEvent(QResizeEvent *event) override; // 重写：窗口大小改变时，调整内部行号栏区域(lineNumberArea)的几何尺寸 (setGeometry)
    void keyPressEvent(QKeyEvent *event) override;  // 重写：拦截键盘输入事件。如果是回车键，则读取上一行的前置空格实现自动缩进；如果是退格键，发射 codeDeleted 信号
